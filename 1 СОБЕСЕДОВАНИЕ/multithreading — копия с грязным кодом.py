...
def save_img(barrier):
    # print("{} вышел из барьера в: {}".format(threading.current_thread().getName(), time.ctime()))
    while True:
...
def collect_links(barrier):
    while True:
        ...
        if not q_links.empty():
            ...
            # imgs = soup.findAll("img", string=[".jpg", ".png"])
            # [imgs_list.append(img['src']) for img in imgs]
            ...
            for img in imgs:
                ...
                # Проверка на раннее скачивание
                # if check_img(img):
                    # # Добавление в очередь картинок
                    # q_imgs.put(img)
                    # Вызов метода скачивания картинки
                    # save_img(img)
